...
protected:
    ...
    // 결과 메시지 출력
    void PrintMessageBox(const string& ascii,const string& title,const string& message);
...
public:
    ...
    // 구매
    BuyResult BuyItem(Player& player, Inventory& inventory, int index);

    // 판매
    bool SellItem(Player& player, Inventory& inventory, int index);

    // 판매 메뉴
    void SellMenu(Player& player, Inventory& inventory, const string& ascii);
